...
def draw_score_time(screen: pg.Surface, score: int, elapsed_time: int) -> None:
    ...
    time_text = font.render(f"Time: {elapsed_time // 50} s", True, (255, 255, 255))  # 50フレーム=1秒
    screen.blit(score_text, (WIDTH - 250, 20))  # スコアを画面右上に表示
    screen.blit(time_text, (WIDTH - 250, 60))  # 時間をその下に表示
...
def main():
    ...
    while True:
        ...
        if hp <= 0:
            ...
            return
        # スコア計算
        score = tmr // 50  # 時間経過に比例してスコア上昇 (50フレームで1秒)
